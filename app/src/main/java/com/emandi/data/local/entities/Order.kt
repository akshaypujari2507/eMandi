...
@Entity(
    tableName = "Order",
    indices = [Index(
        value = ["product_id", "product_name", "isAddToCart", "date_time", "orderID"],
        unique = true
    )]
)
class Order(
    ...
    var totalOrderAmount: Double? = null,
    ...
    override fun toString(): String {
        return "Product(mrp=$mrp, price=$price, product_brand_id=$product_brand_id, product_code=$product_code, product_id=$product_id, " +
                ...
                "isAddToCart = $isAddToCart, date_time = $date_time, totalOrderAmount = $totalOrderAmount, orderID = $orderID)"
    }
}